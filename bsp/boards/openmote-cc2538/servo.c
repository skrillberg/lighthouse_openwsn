...
//rate is in ms. duty_cycle should be 0-100
//d1/2 are pwm 
void motor_init(uint32_t timer,int period, int duty_cycle, uint32_t gpio_base, uint32_t pin){
	 uint32_t pwmTimerClkEnable;
	 uint32_t pwmTimerBase;
	 uint32_t freqCnt=SysCtrlClockGet()*period/1000; //number of ticks in one period is period * clock rate
	 ...
	 uint32_t duty_match_count = timer_cnt * (duty_cycle)/ 100; 	//match set
	 uint16_t match_lower = duty_match_count & 0xFFFF;
	 uint8_t match_upper = (duty_match_count >> 16) & 0xFF;
	 switch(timer){
	  ...
	  IOCPinConfigPeriphOutput(GPIO_D_BASE,GPIO_PIN_1,IOC_MUX_OUT_SEL_GPT0_ICP1); //maps pwm1 output to pin1
	  IOCPinConfigPeriphOutput(GPIO_D_BASE,GPIO_PIN_2,IOC_MUX_OUT_SEL_GPT0_ICP2);
