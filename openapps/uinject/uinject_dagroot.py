...
class moteProbe(threading.Thread):
    ...
    def __init__(self,serialport=None):
        ...
        fieldnames = ['triggered','asn_sent','latency','mote_id']
                            ...
                            try:
                                ...
                                if   self.inputBuf==[ord('R')]:
                                    ...
                                    if self.UINJECT_MASK == ''.join(chr(i) for i in self.inputBuf[-7:]):
                                        ...
                                        counter     = struct.unpack('<B',''.join([chr(b) for b in self.inputBuf[-9:-8]]))[0]
                                        #triggered   = struct.unpack('B',''.join([chr(b) for b in self.inputBuf[-7:-6]]))              
                                        other_stuff = struct.unpack('<BBB',''.join([chr(c) for c in self.inputBuf[-7:-4]]))
                                        mote_id = struct.unpack('<B',''.join([chr(b) for b in self.inputBuf[-8:-7]]))[0]
